...
// TestForwardingWithMoreThanOneHop tests the scenario in which
// more than one forwarding hop is specified.
func (suite *KeeperTestSuite) TestForwardingWithMoreThanOneHop() {
	...
	// Send message to A and verify the result
	result, err := suite.chainA.SendMsgs(transferMsg)
	suite.Require().NoError(err)
	...
	// Receive from B and verify the result
	result, err = pathAtoB.EndpointB.RecvPacketWithResult(packetFromAtoB)
	suite.Require().NoError(err)
	suite.Require().NotNil(result)
	...
	// Send from B to C, receive on C and verify the result
	packetFromBtoC, err := ibctesting.ParsePacketFromEvents(result.Events)
	suite.Require().NoError(err)
	suite.Require().NotNil(packetFromBtoC)
	...
	// Send from C to D, receive on D and verify the result
	...
	// Check that D has received the final amount.
